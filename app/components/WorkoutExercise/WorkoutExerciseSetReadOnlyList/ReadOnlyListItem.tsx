    ...
    >
      {!hideRecords && (
        <View style={{ flex: 1, flexDirection: "row", alignItems: "center" }}>
          ...
          {isRecord && <Icon icon="trophy" color={colors.primary} />}
        </View>
      )}
      {exercise.hasRepMeasument && (
        <SetDataLabel value={set.reps} unit={texts.reps} />
      )}
      {exercise.hasWeightMeasument && (
        <SetDataLabel value={set.weight} unit="kg" />
      )}
      {exercise.hasDistanceMeasument && (
        <SetDataLabel value={set.distance} unit={set.distanceUnit} />
      )}
