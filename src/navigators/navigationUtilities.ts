...
export function useNavigationPersistence(persistenceKey: string) {
  ...
  const onNavigationStateChange = (state: NavigationState | undefined) => {
    ...
    if (state !== undefined) {
      ...
      setValue(persistenceKey, JSON.stringify(state))
    }
  }
  ...
  const restoreState = async () => {
    try {
      ...
      if (!initialUrl) {
        const stateString = getValue(persistenceKey)
        if (stateString) {
          const state = JSON.parse(stateString) as NavigationProps["initialState"]
