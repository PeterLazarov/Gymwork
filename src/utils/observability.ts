...
export function shouldLog(layer: LogLayer, isError: boolean, durationMs: number): boolean {
  const isSlow = durationMs > (getConfig('LOG_SLOW_MS') ?? LOG_CONFIG.SLOW_THRESHOLD_MS);
  
  // Rule 1: Always log errors
  if (isError) return true;

  // Rule 2: Always log slow queries
  if (isSlow) return true;

  // Rule 3: Dev defaults -> Log everything
  if (isDev()) return true;

  // Rule 4: Prod defaults -> Log nothing else (unless flags enabled)
  // Flags override: e.g., LOG_RQ=true in prod
